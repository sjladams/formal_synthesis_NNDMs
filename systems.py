...
# System from: Strategy Synthesis for Partially-known Switched Stochastic Systems (https://arxiv.org/abs/2104.02172)
# Linear - 3 Modes
class JacksonMode1:
    @staticmethod
    def x0(xs):
...
# Non-Linear - SELF definted Alternatives - 4 Modes
...
class CarMode1:
    ...
    def x4(self, xs):
        return (m * xs[3] * xs[4] + Ts * (lf * kf - lr * kr) * xs[5] - Ts * kf * self.delta * xs[3] - Ts * m * (
                    xs[3] ** 2) * xs[5]) / (m * xs[3] - Ts * (kf + kr))

    def x5(self, xs):
        return (Iz * xs[3] * xs[5] + Ts * (lf * kf - lr * kr) * xs[4] - Ts * lf * kf * self.delta * xs[3]) / (
                    Iz * xs[3] - Ts * ((lf ** 2) * kf + (lr ** 2) * kr))


class ImportSystem:
    def __init__(self, system_type: str):
        if system_type == 'linear-slow-center-mode1':
            self.system = LinearSystemSlowCenter()
        elif system_type == 'linear-fast-center-mode1':
        ...
        elif system_type == 'jackson-nl-alt-mode1':
            self.system = JacksonMode1NLAlt()
        elif system_type == 'jackson-nl-alt-mode2':
            self.system = JacksonMode2NLAlt()
        elif system_type == 'jackson-nl-alt-mode3':
            self.system = JacksonMode3NLAlt()
        elif system_type == 'jackson-nl-alt-mode4':
            self.system = JacksonMode4NLAlt()

        elif system_type == 'nonlin-2d-lin-2d-mode1':
            self.system = NonLin2Dlin2DMode1()
        elif system_type == 'nonlin-2d-lin-2d-mode2':
            self.system = NonLin2Dlin2DMode1()
        elif system_type == 'nonlin-2d-lin-2d-mode3':
            self.system = NonLin2Dlin2DMode3()
        elif system_type == 'nonlin-2d-lin-2d-mode4':
            self.system = NonLin2Dlin2DMode4()
        ...
        elif system_type == 'unstable-nl':
            self.system = UnstableNL()

        elif system_type == 'cartpole-stabilizer':
            self.system = CartpoleStabilizer()
        else:
